...
public class TesutEnemySpawner : MonoBehaviour
{
        ...
        for (var i = 0; i < _sponEnemyNum; i++)
        {
            ...
            else if (TesutMap._areas[_randomNumX, _randomNumZ] != null)
            {
                ...
                else
                {
                    ...
                    GameObject _enemy = Instantiate(_enemys[_randoEnemyNum], new Vector3(TesutMap._areas[_randomNumX, _randomNumZ].transform.position.x, 1.5f, TesutMap._areas[_randomNumX, _randomNumZ].transform.position.z), Quaternion.identity);
